...
//    	width: '85px',
        ...
        onComplete : function(ID,fileObj,response,data,event){
            ...
            $("#divimgs").html("<img src=\""+jsonData.fileUrl+"\" width=\"146px\" class=\"avatar\" />");
...
//    }
    ...
    var recommends = document.getElementById("recommendId").getElementsByTagName("select").length;
    var recommendIds="";
    $("#recommendId select").each(function(i,obj){  
		 if(i<recommends)
		 {
...
//    alert(recommendIds);
	$.ajax({
        url:"circleInfoAction_ajaxUpdatequanzi.do",
        type: 'post',
        dataType: 'json',
        data : {
        ...
        success:function(data){
            ...
            alert(data.msg);
            if(data.flag){window.location.href = basePath+"cir2/1/"+$("#circleid").val()+".html";}
        },
        error:function(){
        	$("#wait").css("display","none");
            $("#setting").css("display","block");
...
{
	var tagslis = document.getElementById("tagsli").getElementsByTagName("li").length;
	if(tagslis>=5){$("#newTagsInput").css("display","none");$("#searchTag").hide();return false;}
	$.ajax({
        ...
        success:function(data){
        	...
        	if(circletags.length<=0){
        		$("#searchTag").html("<li style=\"background-color: #e4e4e4; padding-left:10px;\"><a href='javascript:void(0);' onclick='ajaxCreateTag();' style='color:#626262;' >创建标签</a></li>");
